...
let matrix = [];
let matrixAttack = [];
const sizeShip = [3, 2, 1];
const positionArray = ["horizontal", "vertical"]
let quantityShip = [1, 2, 1];
let quantityShipPC = [1, 2, 1];
let ship = {};
let shipRandom = {};
    ...
    if (ship.quantity > 0) {
        ...
        if (ship.position === "horizontal") {
            if ((y + (ship.size - 1)) < 10) {
                ...
                alert("Elige una posicion valida padrino");
            }
        }
        ...
        alert("Ya no tienes de esos mi tibi@");
    }
}
...
function shotPc() {
    ...
    if (matrix[x][y] === "ship") {
        alert("Te dio una funcion random jaja");
        matrix[x][y] = "hit";
        ...
        alert("Te fallo la funcion random");
        matrix[x][y] = "miss";
...
function checkWinner(matrix, player) {
    ...
    if (player === "pc") {
        alert("Te gano pura funcion random XD")
    }
    else {
        alert("Ganaste brother eres un crack!")
    }
}